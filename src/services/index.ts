...
import { useFormatMessage } from "@/components/FormatMessage";
import { validateStatus } from "@/utils/schema";
import { clearAuth } from "@/redux/authentication/slice";
...
const checkErrorNetwork = (err: any) => {
  if (err?.toJSON() && err.toJSON().message === "Network Error") {
    return formatMessage("Server error. Please try again!", { type: "error" });
  }
  return err;
};
...
const checkErrorStatus = (
  ...
  if (
    ...
    if (HTTP_STATUS_CONTSTANTS.SERVER_ERROR !== response?.data?.code) {
      ...
      !options?.isHideErrorMessage &&
        formatMessage(response?.meta?.msg, { type: "error" });
    }
  }
  return response;
};
